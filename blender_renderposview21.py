...
if scene.camera is None:
    ...
    for frame_nr in range(1, 151):
        ...
        # Adjust the camera path based on frame number
        radius = 5 + (frame_nr / 30)  # Increment radius over time
        height_offset = np.sin(np.radians(frame_nr * 2)) * 2  # Vary height using a sine function
        angle_offset = np.radians(frame_nr * 3)  # Change angle over time
        ...
        print(f'Frame {frame_nr}: Position: {pos}, Rotation: {rot_deg}, K: {K.flatten()}')

        # Write to CSV
        csv_writer.writerow([frame_nr, pos.x, pos.y, pos.z, rot_deg[0], rot_deg[1], rot_deg[2], K.flatten()])
